...
A top-level path the tree does not hold falls back to the placement registry (the same lookup `actorOf` uses): an actor this node placed on a worker isolate resolves to its routed handle, and delivery composes the two transports, the wire in and the `MessagePort` across. A tell rides the isolate route as any routed tell does; an ask sends through the route and bridges the route's settlement back to the wire correlation, so the remote asker cannot tell the placement; an envelope pinned to an incarnation re-mints the handle around its own path so the owning worker enforces the pin. Placed handles are never cached on the connection's interned paths, because the owning isolate can change when the name is re-placed; only the registry lookup tracks that. The reverse direction composes through the isolate transport's own target resolution: an envelope from a worker naming a foreign node's path (a placed actor answering a remote sender, or its Terminated to a remote watcher) resolves to the wire-backed handle the seam's sender cache serves, so identity stays stable across both doors. The worker facade adopts the node's advertised address at boot, which is what makes one canonical path space span the wire, the main isolate, and every worker.
